    ...
    decimal? maxPrice)
{
    ...
    // 🔎 FILTER (mới thêm)
        ...
        // ✅ SEARCH
        public async Task<IEnumerable<Property>> SearchAsync(string query, string? userId, string? role)
        {
            var (data, _) = await GetAllAsync(userId, role, 1, int.MaxValue, null, null, null, null);

...
import { isCustomerRole, type CurrentUserProfile } from "@cnl/shared";
    ...
    <RoleGuard allow={isCustomerRole}>
      {(profile) => (
        <ProductShell profile={profile}>
          <CustomerNotificationsContent profile={profile} />
        </ProductShell>
      )}
    </RoleGuard>
  );
...
function CustomerNotificationsContent({ profile }: { profile: CurrentUserProfile }) {
  ...
  return (
    <AppPageContainer>
      <AppPageHeader
        ...
        description="Theo dõi các thay đổi quan trọng từ yêu cầu đã gửi, tiếp nhận, xử lý đến hoàn thành."
      />
